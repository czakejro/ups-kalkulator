...
export default function App(){
  ...
  // Fuel rates
  const fuelPctFull=isKraj?parseFloat(fuelK.replace(",","."))||0:parseFloat(fuelI.replace(",","."))||0;
  const fuelDiscBL=isKraj?50:25;
  const fuelPctBL=fuelPctFull*(1-fuelDiscBL/100); // what BL actually pays

  const w=parseFloat(weight.replace(",","."))||0;
  const upsBase=findPrice(svc,zi,w); // UPS net price (already after BL's negotiated discount)
  ...
  const scDetails=useMemo(()=>{
    ...
    Object.entries(actSC).forEach(([id,on])=>{
      ...
      let cost=0;
      if(sc.type==="cod"){
        const cv=parseFloat((codVal||"0").replace(",","."))||0;
        cost=Math.max(cv*price,isKraj?sc.kMin:sc.iMin);
      } else cost=price;
  ...
  // === COST SIDE (what BL pays UPS) ===
  const fuelOnBaseBL=upsBase!=null?upsBase*(fuelPctBL/100):null;        // fuel on transport (discounted)
  const fuelOnScBL=scDetails.totFuelable*(fuelPctBL/100);               // fuel on fuelable surcharges (discounted)
  const costBL=upsBase!=null?upsBase+fuelOnBaseBL+fuelOnScBL+scDetails.tot:null;

  // === REVENUE SIDE (what client pays) ===
  const sp=parseFloat((sellPrice||"0").replace(",","."))||0;             // price in cennik (without fuel)
  const fuelOnSellFull=sp*(fuelPctFull/100);                             // full fuel surcharge client sees
  const fuelOnScFull=scDetails.totFuelable*(fuelPctFull/100);            // full fuel on surcharges client pays
  const clientTotal=sp>0?sp+fuelOnSellFull+fuelOnScFull+scDetails.tot:null; // total client pays
  ...
  const fuelProfit=upsBase!=null&&sp>0?(sp*(fuelPctFull/100)-upsBase*(fuelPctBL/100))+(scDetails.totFuelable*((fuelPctFull-fuelPctBL)/100)):null;

  // === SUGGESTED SELL PRICE (for target margin) ===
  const tm=parseFloat(tgtM.replace(",","."))||0;
  // clientTotal = sellP + sellP*(fuelFull/100) + scFuelable*(fuelFull/100) + scTot
  // clientTotal = sellP*(1+fuelFull/100) + scConst
  // margin = clientTotal - costBL = tm% * clientTotal
  // clientTotal = costBL / (1 - tm/100)
  // sellP = (clientTotal - scConst) / (1 + fuelFull/100)
  const scConst=scDetails.totFuelable*(fuelPctFull/100)+scDetails.tot;
  const sugClientTotal=costBL!=null&&tm<100?costBL/(1-tm/100):null;
  const sugSellPrice=sugClientTotal!=null?(sugClientTotal-scConst)/(1+fuelPctFull/100):null;
  ...
  const ptable=useMemo(()=>{
    ...
    return svc.prices.map(r=>{
      ...
      const fBL=b*(fuelPctBL/100);
      const fFull=b*(fuelPctFull/100);
      return{kg:r[0],ups:b,fuelBL:fBL,costBL:b+fBL,fuelFull:fFull,clientBase:b+fFull};
    }).filter(Boolean);
  },[svc,zi,fuelPctBL,fuelPctFull]);
        ...
        <div>
          ...
          <Box t="DOPŁATA PALIWOWA">
            ...
            <div style={{fontSize:10,color:"#94a3b8",marginTop:6,display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
              <span>Pełna (klient widzi): <b style={{color:"#e2e8f0"}}>{fuelPctFull.toFixed(2).replace(".",",")}%</b></span>
              <span>Po rabacie BL ({fuelDiscBL}%): <b style={{color:"#22c55e"}}>{fuelPctBL.toFixed(2).replace(".",",")}%</b></span>
            </div>
          </Box>
          ...
          <Box t="PARAMETRY PRZESYŁKI">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:6}}>
              ...
              <Inp l="Cena w cenniku BLPaczka (bez paliw.)" v={sellPrice} s={setSellPrice} p="np. 12,99"/>
            </div>
          </Box>
          ...
          <Box t="KALKULACJA">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}}>
              ...
              <div style={{paddingRight:8,borderRight:"1px solid #1e293b"}}>
                ...
                <R l={`Paliwowa (${fuelPctBL.toFixed(2).replace(".",",")}%)`} v={f2(fuelOnBaseBL)} u="zł" dim/>
              ...
              <div style={{paddingLeft:8}}>
                ...
                <R l={`Paliwowa pełna (${fuelPctFull.toFixed(2).replace(".",",")}%)`} v={sp>0?f2(fuelOnSellFull):"—"} u="zł" dim/>
                {scDetails.d.map(d=><R key={d.id} l={d.name} v={f2(d.cost)} u="zł" dim/>)}
            ...
            {sp>0&&costBL!=null&&marginAmt!=null&&(
              <div style={{marginTop:8,padding:8,background:marginAmt>=0?"rgba(34,197,94,0.06)":"rgba(239,68,68,0.06)",borderRadius:5,border:`1px solid ${marginAmt>=0?"#166534":"#7f1d1d"}`}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4}}>
                  ...
                  <R l="w tym zysk na paliwowej" v={fuelProfit!=null?f2(fuelProfit):"—"} u="zł" c="#f59e0b"/>
                </div>
              </div>
            )}
          </Box>
          ...
          {/* SUGGESTED */}
          <Box t="SUGEROWANA CENA CENNIKOWA">
            ...
            {sugSellPrice!=null&&sugSellPrice>0&&costBL!=null&&(
              <div style={{fontSize:9,color:"#64748b",marginTop:4}}>
                Wpisz w cennik: <b style={{color:"#e2e8f0"}}>{f2(sugSellPrice)} zł</b> → 
                klient z paliwową zapłaci: <b style={{color:"#3b82f6"}}>{f2(sugClientTotal)} zł</b> → 
                koszt BL: <b style={{color:"#ef4444"}}>{f2(costBL)} zł</b> → 
                zysk: <b style={{color:"#22c55e"}}>{f2(sugClientTotal-costBL)} zł</b> ({tgtM}%)
              </div>
            )}
          </Box>
        </div>
            ...
            <div style={{maxHeight:"calc(100vh - 100px)",overflowY:"auto",fontSize:10}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr style={{position:"sticky",top:0,background:"#0f172a",borderBottom:"1px solid #334155"}}>
                  ...
                  <TH r style={{color:"#3b82f6"}}>Z paliw. pełną</TH>
                  <TH r style={{color:"#f59e0b"}}>Zysk paliw.</TH>
                </tr></thead>
                <tbody>{ptable.map((r,i)=>{
                  const act=Math.ceil(w)<=r.kg&&(i===0||Math.ceil(w)>ptable[i-1].kg);
                  const fuelProfit=r.fuelFull-r.fuelBL;
                  return(<tr key={i} style={{background:act?"rgba(245,158,11,0.1)":i%2?"rgba(255,255,255,0.02)":"transparent",borderLeft:act?"3px solid #f59e0b":"3px solid transparent"}}>
                    ...
                    <TD r style={{color:"#93c5fd"}}>{f2(r.clientBase)}</TD>
                    <TD r style={{color:"#fbbf24"}}>{f2(fuelProfit)}</TD>
                  </tr>);
                })}</tbody>
              </table>
            </div>
            <div style={{fontSize:8,color:"#475569",marginTop:6,borderTop:"1px solid #1e293b",paddingTop:4}}>
              Koszt BL = netto + paliwowa po rabacie ({fuelPctBL.toFixed(2).replace(".",",")}%) · Z paliwową pełną = netto + paliwowa pełna ({fuelPctFull.toFixed(2).replace(".",",")}%) · Zysk paliwowy = różnica
            </div>
          </Box>
        </div>
      </div>
...
const Inp=({l,v,s,p})=><div><label style={{fontSize:10,color:"#94a3b8",display:"block",marginBottom:2,textTransform:"uppercase",letterSpacing:1}}>{l}</label><input style={{width:"100%",padding:"6px 8px",background:"#1e293b",border:"1px solid #334155",borderRadius:4,color:"#e2e8f0",fontSize:12,fontFamily:"inherit",boxSizing:"border-box"}} value={v} onChange={e=>s(e.target.value)} placeholder={p}/></div>;
